...
export default function ExpenseTracker(props) {
  ...
  const addExpenseDetails = () => {
    ...
    if (fetchedDate.activeDate && fetchedDate.activeDateType) {
      const currentFilteredExpenses = filteredExpenses(
        fetchedDate.activeDate,
        fetchedDate.activeDateType,
      );
      const isExpenseApplicableInFilter = currentFilteredExpenses.some(
        (expense) => expense.expenseDate === formattedDetails.expenseDate,
      ); // implicit return

      if (!isExpenseApplicableInFilter) {
        showAlert(
          "warning",
          "New expense added but it's outside the current filter. Please clear filter to view all expenses.",
        );
      }
  ...
  useEffect(() => {
    const expensesToConsider =
      fetchedDate.activeDate && fetchedDate.activeDateType
        ? filteredExpenses(fetchedDate.activeDate, fetchedDate.activeDateType)
        : expenseListArrayContents;
    const totalExpenseAmount = expensesToConsider
      .map((expense) => Number(expense.expenseAmount))
  ...
  useEffect(() => {
    if (fetchedDate.activeDate && fetchedDate.activeDateType === "day") {
      showAlert(
        "success",
        `Filter has been set for ${formatDate(fetchedDate.activeDate)}`,
      );
    } else if (
      fetchedDate.activeDate &&
      ...
      showAlert(
        "success",
        `Filter has been set for ${selectedMonth} ${currentYear}`,
      );
    } else if (
      fetchedDate.activeDate &&
        ...
        <form>
          ...
          <div className="form-group">
            ...
            >
              ...
              <option>Shopping Expense</option>
              <option>Travel Expense</option>
              <option>Health Care Expense</option>
              <option>Entertainment Expense</option>
              <option>Investment Expense</option>
              <option>Education Expense</option>
              <option>Donation Expense</option>
                ...
                {filteredExpenses(
                  ...
                  <li className="expenseItem my-4 ml-3" key={index}>
                    ...
                    <div className="container-fluid d-flex justify-content-between align-items-center gap-2 mt-2">
                      <span className="badge bg-info text-white px-2 py-2">
